...
namespace Bigfoot.Web.Core
{
    ...
    public class PostHelper
    {
        ...
        {           
            if (!string.IsNullOrEmpty(Context.QueryString[key]))
                return Context.QueryString[key];

            if (!string.IsNullOrEmpty(Context.Form[key]))
        ...
        public NameValueCollection RequestValues
        {
            ...
            {                
                ...
                // Add the form values
                foreach (var formItem in Context.Form)
                {
                    values.Add(formItem.Key, formItem.Value);
                }
        ...
        public void RequireFileWithExt(string fileExtensions)
        {
            ...
            var ext = Path.GetExtension(Context.Files[0].FileName).ToLowerInvariant();
        ...
        public void ValidateUploadedFilesHaveThisExtension(string fileExtensions)
        {
            ...
            {
                if (string.IsNullOrEmpty(Context.Files[i].FileName.Trim()))
                {
                    continue;
                }
                else
                ...
                var ext = Path.GetExtension(Context.Files[i].FileName).ToLowerInvariant();
